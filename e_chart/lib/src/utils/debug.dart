...
extension ViewDebugExt on ChartView {
  ///=========Debug 绘制相关方法===============
  void debugDraw(Canvas2 canvas, Offset offset, {Color color = const Color(0xFF673AB7), bool fill = true, num r = 6}) {
    if (!kDebugMode) {
      return;
    }
    Paint mPaint = Paint();
  ...
  void debugDrawRect(Canvas2 canvas, Rect rect, {Color color = const Color(0xFF673AB7), bool fill = false}) {
    if (!kDebugMode) {
      return;
    }
    Paint mPaint = Paint();
  ...
  void debugDrawRulerLine(Canvas2 canvas, {Color color = const Color(0xFF000000)}) {
    if (!kDebugMode) {
      return;
    }
    Paint mPaint = Paint();
  ...
  void debugDrawPath(Canvas2 canvas, Path path, {Color color = const Color(0xFF673AB7), bool fill = false}) {
    if (!kDebugMode) {
      return;
    }
    Paint mPaint = Paint();
...
class Logger {
  ...
  static void v(dynamic content, [StackTrace? stackTrace]) {
    _printLog(Level.V, _config.getVerbaseColor(), content, stackTrace);
  }

  static void d(dynamic content, [StackTrace? stackTrace]) {
    _printLog(Level.D, _config.getDebugColor(), content, stackTrace);
  }

  static void i(dynamic content, [StackTrace? stackTrace]) {
    _printLog(Level.I, _config.getInfoColor(), content, stackTrace);
  }

  static void w(dynamic content, [StackTrace? stackTrace]) {
    _printLog(Level.W, _config.getWarningColor(), content, stackTrace);
  }

  static void e(dynamic content, [StackTrace? stackTrace]) {
    _printLog(Level.E, _config.getErrorColor(), content, stackTrace);
  }

  static void e2(dynamic content, [StackTrace? stackTrace]) {
    _printLog(Level.E, _config.getErrorColor(), content, stackTrace);
  }

  static void _printLog(Level level, LogColor color, dynamic content, StackTrace? stackTrace) {
    if (!_config.enablePrint) {
      return;
    }
    if (_config.filter != null) {
      if (_config.filter!.filter(level, content)) {
        return;
      }
    }
    List<String> list = _handleLog(_stringifyMessage(content), stackTrace ?? StackTrace.current);
    for (var s in list) {
  ...
  //生成对应的行数据
  static List<String> _handleLog(String log, StackTrace stackTrace) {
...
abstract class PrintFilter {
  bool filter(Level level, dynamic content);
}
