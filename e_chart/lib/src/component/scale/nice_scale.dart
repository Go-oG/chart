...
NiceScale _nice(
  ...
  if (interval != null && interval > 0) {
    ...
    return NiceScale(min.toDouble(), interval * count.toDouble(), interval.toDouble(), count);
  }
...
NiceScale _standRange(num min, num max, int splitCount, int oldCount, bool startWith0, bool keepCount) {
  ...
  if (max == min) {
    ...
    List<double> list = _adjustAxisRange(min, max, mag);
...
///调整坐标轴的范围和步长值
List<double> _adjustAxisRange(num min, num max, num tickInterval) {
  double temMin = 0;
  if (min < 0) {
    while (temMin > min) {
      temMin -= tickInterval;
  ...
  return [min.toDouble(), max.toDouble(), tickCount.toDouble()];
}
...
class NiceScale {
  ...
  static NiceScale nice(
    ...
    return _nice(min, max, splitNumber,
        ...
        interval: interval,
        keepSplitCount: forceSplitNumber,
        start0: start0);
  }
