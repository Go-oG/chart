...
class QuadTree<T> {
  ...
  QuadTree<T> _addInner(QuadTree<T> tree, num x, num y, T data) {
    ...
    // If the hierarchy is empty, initialize the root as a leaf.
    if (tree._root == null) {
      tree._root = leaf;
      return tree;
    }
  ...
  T? find(int x, int y, [double? r]) {
    ...
    while (quads.isNotEmpty) {
      ...
      if (node.hasChild) {
        ...
        if (node[3] != null) {
          quads.add(InnerQuad(node[3]!, xm.toDouble(), ym.toDouble(), x2, y2));
        }
        if (node[2] != null) {
          quads.add(InnerQuad(node[2]!, x1, ym.toDouble(), xm.toDouble(), y2));
        }
        if (node[1] != null) {
          quads.add(InnerQuad(node[1]!, xm.toDouble(), y1, x2, ym.toDouble()));
        }
        if (node[0] != null) {
...
class QuadNode<T> with AttrMixin {
  ...
  QuadNode({int length = -1, this.data}) {
    if (length != 4 && data == null) {
      throw ChartError('创建时必须选一个');
    }
    if (length == 4 && data != null) {
      throw ChartError('只能选一个');
    }
    if (length != 4) {
      _childMap.clear();
    }
