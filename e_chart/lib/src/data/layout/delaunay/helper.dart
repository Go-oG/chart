...
class Delaunay<T> {
  ...
  double _cx = double.nan;
  double _cy = double.nan;
...
// 跳过重复点附近
      if (k > 0 && (x - xp).abs() <= accuracy && (y - yp).abs() <= accuracy) continue;
...
// 使用边哈希在凸包上找到可见边
  ...
  ///获取相关形状shape
  List<DelaunayNode> getShape(Geom geom, bool triangle) {
    ...
    if (triangle) {
      eachTriangle((p0, p1, p2, index) {
        rl.add(DelaunayNode(
            geom,
            [
              Offset(xFun.call(p0), yFun.call(p0)),
              Offset(xFun.call(p1), yFun.call(p1)),
              Offset(xFun.call(p2), yFun.call(p2)),
            ],
            index: index));
      });
    } else {
      eachVoronoiCell2((p0, p1) {
        rl.add(DelaunayNode(geom, p0.map((e) => Offset(e.x, e.y)).toList(), index: p1));
      });
    }
    return rl;
  }

  void eachShape(bool triangle, void Function(Iterable<Offset2>, int) call) {
    if (triangle) {
      eachTriangle((p0, p1, p2, index) {
        call.call([
          Offset2(xFun.call(p0), yFun.call(p0)),
          Offset2(xFun.call(p1), yFun.call(p1)),
          Offset2(xFun.call(p2), yFun.call(p2)),
        ], index);
      });
    } else {
  ...
  ///遍历沃罗诺伊细胞
  void eachVoronoiCell(void Function(Iterable<Offset2>, int) call) {
    ///存储point ids
    Set<int> seen = {};
  ...
  void eachVoronoiCell2(void Function(Iterable<Offset2>, int) call) {
    Map<int, int> index = {};
    for (var e = 0; e < triangles.length; e++) {
  ...
  ///遍历沃罗诺伊边缘(类似细胞图案)
  void eachVoronoiEdge(void Function(Offset2, Offset2, int) call) {
    for (var e = 0; e < triangles.length; e++) {
      if (e < halfEdges[e]) {
  ...
  ///给定三角形索引 计算其中心
  Offset2 triangleCenter(int triangleIndex) {
    List<Offset2> vertices = List.from(triangleToPoints(triangleIndex).map((p) => points[p]));
...
///计算圆心
Offset2 _circumCenter(num ax, num ay, num bx, num by, num cx, num cy) {
  var dx = bx - ax;
  var dy = by - ay;
  var ex = cx - ax;
  var ey = cy - ay;
