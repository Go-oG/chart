...
class CollideForce extends GForce {
  ...
  ///力的强度会抑制速度的变化，因此可以同时将不同的[GForceSimulation]混合在共同使用。
  num _strength = 1;
  ...
  ///半径函数，返回每个节点的半径大小
  GForceFun _radiusFun = (node, i, nodes, w, h) {
    return 1;
  };
  ...
  CollideForce({GForceFun? radiusFun, int? iterations, double? strength}) {
    if (radiusFun != null) {
  ...
  void force([double alpha = 1]) {
    ...
    for (int k = 0; k < _iterations; ++k) {
      QuadTree<GraphNode> tree =
          QuadTree.simple<GraphNode>((d) => d.x + d.vx, (d) => d.y + d.vy, _nodes).eachAfter(_prepare);
  ...
  CollideForce radius(GForceFun fun) {
    _radiusFun = fun;
    _initialize();
    return this;
  }
