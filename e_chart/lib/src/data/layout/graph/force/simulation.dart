...
class GForceSimulation extends ViewNotifier {
  ...
  GForceSimulation(this.context, Graph graph) {
  ...
  GForceSimulation tick({int iterations = 1, bool byUser = true}) {
  ...
  GForceSimulation stop() {
    if (_controller != null) {
      context.removeAnimate(_controller);
      _controller = null;
    }
    _optCount = 0;
    _optSD = 1;
  ...
  GForceSimulation nodes(List<GraphNode> list) {
    _graph.nodes.clear();
  ...
  GForceSimulation setGraph(Graph graph) {
    _graph.nodes.clear();
  ...
  ///alpha范围为 [0,1]
  GForceSimulation alpha(double value) {
    _alpha = value;
    return this;
  }
  ...
  GForceSimulation alphaMin(double value) {
    _alphaMin = value;
    return this;
  }
  ...
  GForceSimulation alphaDecay(double value) {
    if (value < 0 || value > 1) {
  ...
  GForceSimulation alphaTarget(double value) {
    if (value < 0 || value > 1) {
  ...
  GForceSimulation velocityDecay(double value) {
    if (value < 0 || value > 1) {
  ...
  ///默认为[DefaultLCG]。
  GForceSimulation randomSource(LCG lcg) {
    _random = lcg;
    for (var value in _forces) {
      _initializeForce(value);
    }
  ...
  GForceSimulation addForce(GForce f) {
    removeForce(f);
    _forces.add(_initializeForce(f));
    return this;
  }

  GForceSimulation removeForce(GForce f) {
    _forces.remove(f);
    return this;
  }
  ...
  GForce _initializeForce(GForce force) {
