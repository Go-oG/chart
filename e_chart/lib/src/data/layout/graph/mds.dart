...
class GMDSTransform extends GTransform {
  List<SNumber> center;
  ...
  void transform(Context context, double width, double height, Graph? graph) {
    ...
    ///MDS
    List<Offset2> positions = runMDS(scaledDistances);
  ...
  ///输出值代表样本的低位坐标
  List<Offset2> runMDS(List<List<double>> distanceList, [int dimension = 2]) {
    try {
      ///构建多维空间距离矩阵
