...
PackCircle packEncloseRandom(List<TreeNode> circles, LCG random) {
  shuffle(circles, random);
  int i = 0, n = circles.length;
  List<PackCircle> B = [];
  PackCircle? e;
  while (i < n) {
    TreeNode p = circles[i];
    var pc = PackCircle(p.x, p.y, p.r);
...
List<PackCircle> extendBasis(List<PackCircle> B, PackCircle pc) {
  ...
  // If we get here then B must have at least one shape.
